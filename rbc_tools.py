...
class MyFunctionSpace:
    ...
    def generate(self, params=None):
        ...
        self.V = VectorElement("CG",self.geometry.mesh.ufl_cell(), degree = 1)
        self.P = FiniteElement( "CG",self.geometry.mesh.ufl_cell(), degree = 1)
        self.E = FiniteElement( "CG",self.geometry.mesh.ufl_cell(), degree = 1)
...
class MySolver:
    def __init__(self, geometry, function_space, params=None):
        ...
        self.pr = 1
        self.Ra = 2E6
        self.u_0  = Expression(('0','0','0.0','1.0'),degree = 2)
    ...
    def step_forward(self,input,const,amp):
        '''
        input batch y,x,4
        const batch, nstep
        amp batch n_step
        '''
        batch  =const.shape[0]
        output_fem = torch.zeros_like(input)
        for i in range(batch):
            self.w.vector()[:] = input[i,:,:,:].reshape(-1)[dof_to_vertex_map(self.W)[:-4*(self.geometry.n+1)].astype('int32')].clone().detach().cpu().numpy()
            self.w_old.vector()[:] = input[i,:,:,:].reshape(-1)[dof_to_vertex_map(self.W)[:-4*(self.geometry.n+1)].astype('int32')].clone().detach().cpu().numpy()
            self.bc_bottom.a = const[i,0]
            self.bc_bottom.b = amp[i,0]
            self.solver.solve()
            output_fem[i,:,:,:] = torch.Tensor(self.w.vector()[:][vertex_to_dof_map(self.W)].reshape(self.geometry.n+1,2*self.geometry.n+1,4)).to(self.device)
        return output_fem
