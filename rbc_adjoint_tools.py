...
class PeriodicBoundary(SubDomain):
    ...
    def inside(self, x, on_boundary):
        return bool(x[0] < 0.0000000001 and x[0] > -0.000000001 and on_boundary)
    def map(self, x, y):
        y[0] = x[0] - 2.0
        y[1] = x[1] 
...
class MySolver:
    def __init__(self, geometry, function_space, params=None):
        ...
        self.pr = 1
        self.Ra = 2e6
        self.u_0  = Expression(('0','0','0','1'),degree = 2)
    ...
    def generate_bc(self,a1 = None,b1 = None):
        self.bndry = self.geometry.bndry
        if a1 == None:
            self.a = np.random.rand(1)*0.4 + 1.8
            self.b = np.random.rand(1)*0.05+0.03
            self.bc_bottom = Expression('a+b*sin(2*pi*x[0])',a = self.a[0],b = self.b[0],pi = np.pi,degree = 2)
            self.bc_top = Expression('0.0',degree = 2)
        else:
            
            self.bc_bottom = Expression('a+b*sin(2*pi*x[0])',a = a1,b = b1,pi = np.pi,degree = 2)
            self.bc_top = Expression('0.0',degree = 2)
        ...
        self.bcs = [self.bcv_t, self.bcv_b, self.bct_t, self.bct_b]

    

    def generate_solver(self,w_tn =None , w_tnp1 = None,data = None):
        (self.v_, self.p_, self.e_) = TestFunctions(self.W)
        self.w = Function(self.W)
        if w_tn != None:
            self.w.vector()[:] = w_tn.reshape(-1)[dof_to_vertex_map(self.W)[:-4*(self.geometry.n+1)].astype('int32')].detach().cpu().numpy()
        else:
        ...
        if w_tn != None:
            self.w_old.vector()[:] = w_tn.reshape(-1)[dof_to_vertex_map(self.W)[:-4*(self.geometry.n+1)].astype('int32')].detach().cpu().numpy()
        else:
            self.w_old.interpolate(self.u_0)
        (self.v_old, self.p_old, self.e_old) = split(self.w_old)
        if data == 1:
            self.w_data = Function(self.W)
            self.w_data.vector()[:] = w_tnp1.reshape(-1)[dof_to_vertex_map(self.W)[:-4*(self.geometry.n+1)].astype('int32')].detach().cpu().numpy()
            self.w_c = Control(self.w_old)
            self.w_c1 = Control(self.w_data)
        elif data == 0:
            self.w_data = Function(self.W)
            self.w_data.vector()[:] = w_tnp1.reshape(-1)[dof_to_vertex_map(self.W)[:-4*(self.geometry.n+1)].astype('int32')].detach().cpu().numpy()
    ...
    def get_obs(self):
        ...
        return temp, velo, p ,self.a , self.b
    def plot_all(self):
        ...
        plt.colorbar()
        plt.savefig('./img_temp/pic-{}.png'.format(self.epoch))

        fig = plt.figure()
        plt.axis('equal')
        plt.contourf(self.meshgrid[0],self.meshgrid[1],p, 200, cmap='jet')
        plt.colorbar()
        plt.savefig('./img_pressure/pic-{}.png'.format(self.epoch))
        ...
        plt.savefig('./img_velo/pic-{}.png'.format(self.epoch))

    def set_solver(self,w_tn,w_tnp1):
    ...
    def compute_loss(self):
        ...
        grad_e, _ = compute_gradient(J,[self.w_c,self.w_c1])
